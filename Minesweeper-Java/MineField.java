...
class MineField{
    ...
    //method for random placing of 15 mines
    ...
    //a method for displaying the map after treading on a mine field
    private void boom() {
	for(int row=0;row<rowMax;row++){
	    for(int col=0;col<colMax;col++){
		if(mines[row][col]){
	...
	if(visible[row][col]){
	    ...
	    return drawUnvisible();
	    
	}
	
	return switchCount(count);
    }
    
    // a help method for drawChar 
    private char drawUnvisible(){
    	if(boom){
    		return '-';
    	    }
    	else
    ...
    //a help method for printing neighbouring mine fields
    public char switchCount(int count){
	switch(count){
	case 0:return '0';
	case 1:return '1';
	case 2:return '2';
